...
int main()
{
    ...
    auto router = http::static_router::make<my_message_t>()
        .append_route("hey", [](const string &url, my_message_t &imessage) {
            cout << "hey" << endl;
            http::static_router::done();
        })
        // FIXME: right now the order is inverted
        .append_route("listen", [](const string &url, my_message_t &imessage) {
            cout << "listen2" << endl;
            http::static_router::done();
        })
        .append_route("listen", [](const string &url, my_message_t &imessage) {
            if (url[0] != ' ')
                return http::static_router::next(url, imessage);

            cout << "listen" << endl;
            http::static_router::done();
        })
        .append_route("link", [](const string &url, my_message_t &imessage) {
            cout << "link" << endl;
            http::static_router::done();
        });

    router.do_easy_thing("  listen  ", imessage);
    router.do_easy_thing("0  listen  ", imessage);
